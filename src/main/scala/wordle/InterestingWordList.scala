...
// From https://github.com/nimblebun/wordle-cli/blob/master/words/word_list.go
object InterestingWordList {
  val words = Seq(
  ...
  def randomWord = words(Math.abs(new Random().nextInt) % words.length)
}
